...
function loadInboundData() {
  ...
  if (tableContainer) {
    ...
    .catch(err => {
      ...
      // KORREKTUR: Keine Inline-Styles, Nutzung der globalen Fehler-Klasse aus tables.css
...
// 3. INITIALISIERUNG ÜBER DIE ZENTRALE LADEKETTE
    ...
    inboundForm.addEventListener("submit", (e) => {
      ...
      if (!productSelect || !productSelect.value || !dateInput || !dateInput.value || !priceInput || !priceInput.value) {
        if (errorEl) {
          ...
          errorEl.style.display = "block"; // Sichtbarkeit gesteuert
  ...
  if (toggleHeader) {
    toggleHeader.addEventListener("click", () => {
      ...
      if (form.style.display === "none") {
        ...
        btn.removeAttribute("data-state"); // Nutzt das native CSS des Themes
      } else {
        form.style.display = "none";
        btn.setAttribute("data-i18n", "btn_expand");
        btn.setAttribute("data-state", "collapsed"); // Übergibt die Steuerung ans Stylesheet
      }

...
-- -----------------------------------------------------------------------------
-- TABLE 0: PRODUCT CATEGORIES (Dynamic product categories isolated per tenant)
-- -----------------------------------------------------------------------------
CREATE TABLE product_categories (
    category_id SERIAL PRIMARY KEY,
    tenant_id INT NOT NULL,                  -- Boundary for multi-tenancy isolation
    category_code VARCHAR(50) NOT NULL,      -- Internal machine code (e.g., 'TCG', 'WINE')
    display_name VARCHAR(100) NOT NULL,     -- Human readable string (e.g., 'Trading Card Games')
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (tenant_id, category_code)        -- Prevents duplicate codes within the same workspace
);
...
CREATE TABLE product_master (
    ...
    tenant_id INT NOT NULL,                  -- Boundary for multi-tenancy isolation
    category_id INT REFERENCES product_categories(category_id) ON DELETE SET NULL, -- Dynamic link replacing old static string
...
CREATE INDEX idx_categories_tenant ON product_categories(tenant_id);          -- Fast multi-tenant category lookups
